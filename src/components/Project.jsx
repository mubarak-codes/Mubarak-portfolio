   ...
   <section className="mt-5 px-4 px-sm-5" id="projects">
    ...
    <div className=" gap-4 d-grid gr-2-sm">
     {projects.map((project)=>(
      <div className={`bg-success bg-opacity-10 border border-2 border-success border-opacity-50 rounded-4  d-flex p-3  ${text}`} key={project.id}>
       <div className="">
        ...
        <a href={project.link} className={`mfs-1 ${text} opacity-75 click-fade click-grow hover-text-underline`} target="_blank">Visit Website</a>
       </div>
      </div>
     ))}
    </div>
