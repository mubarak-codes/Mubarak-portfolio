const Services = () => {
 return (
  <>
   <section id="services" className="mt10 ps10-sm ps19-md px4 opacity-75">
    <h2 className="mb-3 text-2xl txt-bolder txt-center txt-start-sm ff-montserrat-bold">Services</h2>
    <div className="flex d-col nowrap-sm gap2">
     {/*front end Developer*/}
      <div className=" fs4 mb3 bg-gray-500/20 p4 hover-grow hover-shadow3">
       <h3 className='fs3 txt-bold'>Front-End Developer</h3>
       <div className="fs1">Crafting responsive, interactive web experiences</div>
  </div>

    {/*back end Developer*/}
      <div className=" fs4 mb3 bg-gray-500/20 p4 hover-grow hover-shadow3">
      <h3 className="fs3 txt-bold">Back-End Developer</h3>
      <div className="fs1">Building secure APIs, servers & database systems</div>
  </div>
    </div>
   </section>
  </>
  )
}

export default Services;
